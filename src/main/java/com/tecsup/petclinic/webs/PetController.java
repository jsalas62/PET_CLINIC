...
public class PetController {
	...
	@GetMapping("/findByBirthMonth/{month}")
	public ResponseEntity<List<PetTO>> findPetsByBirthMonth(@PathVariable int month) {
		List<Pet> pets = petService.findByBirthMonth(month);
		if (pets.isEmpty()) {
			return ResponseEntity.noContent().build();
		}
		List<PetTO> petsTO = mapper.toPetTOList(pets);
		return ResponseEntity.ok(petsTO);
	}
